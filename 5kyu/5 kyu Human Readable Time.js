...
function humanReadable(seconds) {
    ...
    let rSeconds = seconds % 60;
  
    // Форматируем часы, минуты и секунды, добавляя ведущие нули
    let fHours = hours.toString().padStart(2, '0');
    let fMinutes = minutes.toString().padStart(2, '0');
    let fSeconds = rSeconds.toString().padStart(2, '0');
  
    // Соединяем отформатированные значения двоеточием
